...
%%
fid=fopen('mf.txt','rt')
meanflow=fscanf(fid,'%s',1);
fclose(fid);
%%
% determine size of snapshots
fid=fopen(meanflow,'rt');
n=fscanf(fid,'%d',1);
b=textscan(fid,'%f',n);
mf=b{1};
fclose(fid);
...
fid=fopen(strcat('PODMF/eigs.txt'),'wt');
...
for j=1:p	% save direct balanced modes
    j
    fid=fopen(strcat('PODTIMEMF/mode_',num2str(j),'_.txt'),'wt');
    fprintf(fid,'%d\n',nbre+1);
    fprintf(fid,'%21.14e\n',sqrt(S(j,j))*V(:,j));
    fclose(fid);
end
...
for i=0:nbre	% loop on snapshots of direct simulation
    ...
    fclose(fid);
    v=v+(b{1}-mf)*V(i+1,1:p)*S(1:p,1:p)^(-1/2);	% update balanced mode
end
    
for j=1:p	% save direct balanced modes
    j
    fid=fopen(strcat('PODMF/mode_',num2str(j),'_.txt'),'wt');
    fprintf(fid,'%d\n',n);
    fprintf(fid,'%21.14e\n',v(:,j));
    fclose(fid);
end

save('PODMF.mat', 'v', '-v7.3');
