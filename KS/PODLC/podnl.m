...
fid=fopen(strcat('PODNL/eigs.txt'),'wt');
...
for i=0:nbre	% loop on snapshots of direct simulation
    i
    ii=i*step;
    fid=fopen(strcat('../DNS2/vect2nl/nl_',num2str(100000+in+ii+1),'_.txt'),'rt');
    num=fscanf(fid,'%d',1);
...
for j=1:p	% save direct balanced modes
    j
    fid=fopen(strcat('PODNL/mode_',num2str(j),'_.txt'),'wt');
    fprintf(fid,'%d\n',n);
    fprintf(fid,'%21.14e\n',vNL(:,j));
    fclose(fid);
end

save('PODNL.mat', 'vNL', '-v7.3');
