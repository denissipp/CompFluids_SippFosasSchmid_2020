...
fid=fopen(strcat('PODNLMF/eigs.txt'),'wt');
...
for i=0:nbre	% loop on snapshots of direct simulation
    i
    ii=i*step;
    fid=fopen(strcat('vect2nlmf/nl_',num2str(100000+ii+in+1),'_.txt'),'rt');
    num=fscanf(fid,'%d',1);
...
for j=1:p	% save direct balanced modes
    j
    fid=fopen(strcat('PODNLMF/mode_',num2str(j),'_.txt'),'wt');
    fprintf(fid,'%d\n',n);
    fprintf(fid,'%21.14e\n',vNL(:,j));
    fclose(fid);
end

save('PODNLMF.mat', 'vNL', '-v7.3');
