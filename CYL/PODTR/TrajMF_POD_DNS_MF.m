...
fid=fopen(strcat('../TRMF/vect2/cbf_100001_.txt'),'rt');
num=fscanf(fid,'%d',1)
fclose(fid)
...
for i=1:nbre+1    % loop on snapshots of direct simulation
    i
    ii=(i-1)*step;
    fid=fopen(strcat('../TRMF/vect2/cbf_',num2str(100000+ii+1),'_.txt'),'rt');
    num=fscanf(fid,'%d',1);
    b=textscan(fid,'%f',num);
...
%%
save('SnapMF_State_POD_MF.mat','yyMF','DNSMF_t','dt','step');
